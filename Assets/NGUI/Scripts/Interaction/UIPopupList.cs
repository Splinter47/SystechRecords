...
public class UIPopupList : UIWidgetContainer
{
	...
	public bool isSamsVersion = false;
	...
	public void Close ()
	{
		if (mChild != null)
		{
			...
			if(isSamsVersion){
				...
				UILabel text = textObject.GetComponent<UILabel>();
		...
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && atlas != null && isValid && items.Count > 0)
		{
			...
			for (int i = 0, imax = items.Count; i < imax; ++i)
			{
				...
				if(isSamsVersion){
					...
					float bX = lbl.transform.localPosition.x;
					float bY = lbl.transform.localPosition.y;
					float bZ = lbl.transform.localPosition.z;
					lbl.transform.localPosition = new Vector3(bX + 85, bY, bZ);
			...
			mBackground.height = Mathf.RoundToInt(-y + bgPadding.y) + 45;


			//********************/new/*********************//
			if(isSamsVersion){
				GameObject backgroundParent = mBackground.gameObject;
				float bX = backgroundParent.transform.localPosition.x;
				float bY = backgroundParent.transform.localPosition.y;
				float bZ = backgroundParent.transform.localPosition.z;
				backgroundParent.transform.localPosition = new Vector3(bX, bY + 45, bZ);
